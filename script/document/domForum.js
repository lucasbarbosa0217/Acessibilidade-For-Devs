...
ForumPostsService.readMe()
  ...
  .catch((error) => {
    window.alert("Faça login para acessar essa área");
    window.location.href = "./login.html";
  });
  ...
  data.forEach((element) => {
    ...
    post = `
        ...
        <div class="contentforum">
            <hr>
            <P>${pequeno}...</P>
            <p class="infoforum">Pergunta feita por <b id="forumusername">${
              element.author.name
            }</b>, <b id="forumdate">${formatDate(element.createdAt)}</b></p>
...
function configuraComentario(comentario) {
  conteudo = `
    <div class="container answer">
        ...
        <p class="infoforum">Resposta feita por <b id="forumusername">${
          comentario.author.name
        }</b>, <b id="forumdate">${formatDate(comentario.updatedAt)}</b></p>
    </div>
    `;
